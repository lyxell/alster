...
int lua_buffer_sub(lua_State *L) {
    ...
    if (lua_gettop(L) == 3) {
        ...
        assert(lua_isnumber(L, -2));
        assert(lua_isuserdata(L, -3));
        orig = (buf*) lua_touserdata(L, -3);
        skip = lua_tointeger(L, -2) - 1;
        length = lua_tointeger(L, -1) - lua_tointeger(L, -2) + 1;
    } else {
        assert(lua_isnumber(L, -1));
        assert(lua_isuserdata(L, -2));
        orig = (buf*) lua_touserdata(L, -2);
        skip = lua_tointeger(L, -1) - 1;
        length = orig->num_lines - skip;
    }
    buf* b = (buf*) lua_newuserdata(L, sizeof(buf)+length*sizeof(char32_t*));
    b->num_lines = length;
    for (size_t i = skip; i < skip + length; i++) {
        b->lines[i - skip] = orig->lines[i];
    }
...
int lua_buffer_len(lua_State *L) {
    ...
    assert(lua_isnil(L, -1));
    assert(lua_isuserdata(L, -2));
    buf* b = (buf*) lua_touserdata(L, -2);
    lua_pushinteger(L, b->num_lines);
    return 1;
}
...
int lua_buffer_concat(lua_State *L) {
    ...
    assert(lua_isuserdata(L, -1));
    assert(lua_isuserdata(L, -2));
    auto b1 = (buf*) lua_touserdata(L, -2);
    auto b2 = (buf*) lua_touserdata(L, -1);
    buf* b = (buf*) lua_newuserdata(L, sizeof(buf)+(b1->num_lines+b2->num_lines)*sizeof(char32_t*));
    b->num_lines = b1->num_lines + b2->num_lines;
    for (size_t i = 0; i < b1->num_lines; i++) {
        b->lines[i] = b1->lines[i];
    }
    for (size_t i = 0; i < b2->num_lines; i++) {
        b->lines[i+b1->num_lines] = b2->lines[i];
    }
    luaL_getmetatable(L, "buffer");
    lua_setmetatable(L, -2);
    return 1; 
