...
alster: alster_re2c.cpp buffer.cpp tokenize/tokenize_re2c.cpp buffer.h syntax/c.re2c
	$(CXX) $(CXXFLAGS) \
		alster_re2c.cpp \
		buffer.cpp \
		tokenize/tokenize_re2c.cpp \
		$(LDFLAGS) -o $@
...
fuzzer: alster.cpp buffer.cpp tokenize/tokenize_c.cpp buffer.h
	clang++ -DFUZZ -std=c++17 -g -O1 \
		...
		buffer.cpp tokenize/tokenize.cpp $(LDFLAGS) -o $@
